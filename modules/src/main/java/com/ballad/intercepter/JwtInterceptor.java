...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws IOException {
        ...
        try {
            ...
            e.printStackTrace();
            baseResponse = ResultUtils.error(ResultCode.TOKEN_INVALID_SIGNATURE);
        } catch (TokenExpiredException e) {
            e.printStackTrace();
            baseResponse = ResultUtils.error(ResultCode.TOKEN_EXPIRED);
        } catch (AlgorithmMismatchException e) {
            e.printStackTrace();
            baseResponse = ResultUtils.error(ResultCode.TOKEN_ALG_EXPECTION);
        } catch (Exception e) {
            e.printStackTrace();
            baseResponse = ResultUtils.error(ResultCode.TOKEN_INVALID);
        }
        //将map转为json
